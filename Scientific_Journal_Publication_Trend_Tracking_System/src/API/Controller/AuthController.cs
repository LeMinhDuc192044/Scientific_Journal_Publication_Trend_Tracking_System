...
public class AuthController : ControllerBase
{
    ...
    public AuthController(IMediator mediator, ICurrentUserService currentUserService, IUnitOfWork unitOfWork)
    {
        _mediator = mediator;
    ...
    {
        var command = new RegisterCommand(request.Email, request.FullName, request.Password, request.Role);
        var result = await _mediator.Send(command, cancellationToken);
        return CreatedAtAction(nameof(Register), ApiResponse<RegisterResponse>.Created(result));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await _mediator.Send(command, cancellationToken);
        return Ok(ApiResponse<RefreshTokenResponse>.Ok(result, "Token refreshed successfully"));
    }
    ...
    public async Task<IActionResult> Logout(CancellationToken cancellationToken)
    {
        ...
        if (userId == null)
        {
            return Unauthorized(ApiResponse.Failure("User not authenticated", 401));
        }
